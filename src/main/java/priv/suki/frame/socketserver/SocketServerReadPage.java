...
/**
 * socket_server接口参数说明界面
 *
 * @author 花小琪
 * @version 1.0
 */
public class SocketServerReadPage {
    // Log
    private static Log log = LogFactory.getLog(SocketServerReadPage.class);
    private static SocketServerReadPage socketServerReadPage = new SocketServerReadPage();
    private static JFrame InfoSend;

    /**
     * SocketServerReadPage构造模块，初始化界面
     */
    public SocketServerReadPage() {
        Thread.currentThread().setName("SocketServerReadPage");
        initialize();
    }
    ...
    /**
     * 获取SocketServerReadPage对象
     *
     * @return SocketServerReadPage对象
     */
    public static SocketServerReadPage initInterfacePage() {
        return socketServerReadPage;
    }

    /**
     * 打开SocketServerReadPage界面，每次获取时，更新接口选择框显示
     */
