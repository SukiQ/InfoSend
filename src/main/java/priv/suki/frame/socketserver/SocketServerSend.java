...
public class SocketServerSend {
    // Log
    private static Log log = LogFactory.getLog(SocketServerSend.class);
    //	private static Logger logger = Logger.getLogger("Log4jMain");
    private static SocketServerSend SocketServerSend = new SocketServerSend();
    ...
    /**
     * SocketServerSend构造模块，初始化界面
     */
    public SocketServerSend() {
        initialize();
    }
    ...
    private void initialize() {
        ...
        table_param.setModel(new DefaultTableModel(
                new Object[][]{{"port", Propert.getPropert().getSocket_server_port()},
        ...
        btnSend.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ...
                    /* 设置报错弹窗 */
                    log.error("socket_server接口初始化发送异常", t);
    ...
    /**
     * 获取SocketServerSend对象
     *
     * @return SocketServerSend对象
     */
    public static SocketServerSend initInterfacePage() {
        return SocketServerSend;
    }

    /**
     * 打开SocketServerSend界面，每次获取时，更新参数表显示
     */
