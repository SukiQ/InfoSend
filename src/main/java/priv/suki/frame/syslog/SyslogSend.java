...
public class SyslogSend {
     ...
     * log
     */
    private static Log log = LogFactory.getLog(SyslogSend.class);
    private static SyslogSend SyslogSend = new SyslogSend();
    ...
    private void initialize() {
        ...
        table_param.setModel(new DefaultTableModel(
                new Object[][]{{"type", Propert.getPropert().getSyslog_type()},
                        {"port", Propert.getPropert().getSyslog_port()},
     ...
     * @return SyslogSend对象
     */
    public static SyslogSend initInterfacePage() {
        return SyslogSend;
    }

    /**
     * 打开SyslogSend界面，每次获取时，更新参数表显示
     */
